{}: let
  lan = import ./known-hosts.nix;
in {
  defaultTTL = 86400;
  zones = {
    "${lan.hosts.framework.domainName}" = {
      "" = {
        a = {
          data = [lan.hosts.framework.ip];
          ttl = 60;
        };
      };
    };
    "gamer.local" = {
      "" = {
        a = {
          data = [lan.hosts.gamer.ip];
          ttl = 60;
        };
      };
    };
    "router.local" = {
      "" = {
        a = {
          data = [lan.hosts.router.ip];
          ttl = 60;
        };
      };
    };
    "hive.local" = {
      "" = {
        a = {
          data = [lan.hosts.hive.ip];
          ttl = 60;
        };
      };
      "redirect".cname.data = "zaks.pw";
    };

    # "example.com" = {
    #   "" = {
    #     soa = {
    #       data = {
    #         rname = "admin.example.invalid";
    #         mname = "ns.example.invalid";
    #         serial = 1970010100;
    #         refresh = 7200;
    #         retry = 3600;
    #         ttl = 60;
    #         expire = 1209600;
    #       };
    #     };
    #     ns = {
    #       data = [
    #         "ns1.invalid"
    #         "ns2.invalid"
    #         "ns3.invalid"
    #       ];
    #     };
    #     txt = {
    #       data = [
    #         "meow"
    #         "v=spf1 a:mail.example.com -all"
    #       ];
    #     };
    #   };
    #   "mail._domainkey".txt.data = "v=DKIM1; k=rsa; p=; s=*;";
    # };
    # "example.net" = {
    #   "" = {
    #     a = {
    #       data = ["203.0.113.73"];
    #       ttl = 60;
    #     };
    #   };
    # };
    # "example.invalid" = {
    #   "" = {
    #     a = {
    #       data = ["198.51.100.35"];
    #       ttl = 60;
    #     };
    #     aaaa = {
    #       data = ["2001:DB8:42fc::64"];
    #       ttl = 60;
    #     };
    #   };
    #   "redirect".cname.data = "example.net";
    # };
  };
}
